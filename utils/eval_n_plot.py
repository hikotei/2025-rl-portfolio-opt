...
def compute_strategy_metrics(
    ...
    for model_name, df in drl_dfs.items():
        ...
        # for max drawdown instead of mean we should get overall max
        results[f"DRL_{model_name}"]["Max Drawdown"] = pd.DataFrame(metrics)["Max Drawdown"].min()
    ...
    for lb in lookbacks:
        ...
        # for max drawdown instead of mean we should get overall max
        results[f"MVO_{lb}"]["Max Drawdown"] = pd.DataFrame(metrics)["Max Drawdown"].min()
    ...
    # for max drawdown instead of mean we should get overall max
    results["Naive"]["Max Drawdown"] = pd.DataFrame(metrics)["Max Drawdown"].min()
    ...
    # for max drawdown instead of mean we should get overall max
    results["Buy_n_Hold"]["Max Drawdown"] = pd.DataFrame(metrics)["Max Drawdown"].min()
...
def plot_drl_portfolios(
    df_drl_port: pd.DataFrame,
    save_dir: str = None,
    fname: str = None,
    title: str = None,
):
    ...
    plt.figure(figsize=(15, 5))
    palette = sns.color_palette(
        "viridis", as_cmap=False, n_colors=df_drl_port["year"].nunique()
    )
    sns.lineplot(
        data=df_drl_port,
        x="trading_day",
    ...
    # horizontal line at initial value
    plt.axhline(y=100_000, color="gray", linestyle="--", label="Initial Value")

    if title is not None:
        plt.title(title)
    plt.xlabel("Trading Day")
    plt.ylabel("Portfolio Value")
    plt.legend(title="Backtest Year", bbox_to_anchor=(1.01, 1), loc="upper left")
    plt.tight_layout()
