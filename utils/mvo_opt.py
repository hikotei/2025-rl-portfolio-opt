...
class MVOOptimizer:
    ...
    def __init__(self, tickers, lookback=60, risk_free_rate=0.0):
        """
        Initialize the MVOOptimizer with asset tickers, lookback window, and annual risk-free rate.
        
        Parameters:
            tickers (list): Asset tickers to include in the portfolio.
            lookback (int): Number of days for the historical lookback window.
            risk_free_rate (float): Annualized risk-free rate used in Sharpe ratio calculations.
        """
    ...
    def reset(self):
        """
        Reset the optimizer's internal state, clearing cash, share holdings, portfolio value, and history.
        """
        self.cash = 0
    ...
    def negative_sharpe_ratio(weights, mean_returns, cov_matrix, risk_free_rate):
        """
        Compute the negative Sharpe ratio for a portfolio given weights, mean returns, covariance matrix, and risk-free rate.
        
        Parameters:
            weights (array-like): Portfolio weights for each asset.
            mean_returns (array-like): Expected returns for each asset.
    ...
    def get_weights(self, return_window, method="pypfopt"):
        """
        Compute optimal portfolio weights for a given window of asset returns using mean-variance optimization.
        
        Parameters:
            return_window (pd.DataFrame): DataFrame of historical returns for each asset in the lookback window.
            method (str): Optimization method to use; "pypfopt" for PyPortfolioOpt or "scipy" for SciPy-based optimization.
        
        Returns:
            dict: Mapping of asset tickers to portfolio weights, or None if input data contains NaNs.
        """
    ...
    def backtest(self, df_ret, df_prices, start_date, end_date, initial_cash=100_000, method="pypfopt"):
        """
        Simulate daily portfolio rebalancing and performance over a specified period using mean-variance optimization.
        
        Parameters:
            df_ret (pd.DataFrame): DataFrame of daily asset returns, indexed by date.
            df_prices (pd.DataFrame): DataFrame of daily asset prices, indexed by date.
            start_date (str or pd.Timestamp): Start date for the backtest period.
            end_date (str or pd.Timestamp): End date for the backtest period.
            initial_cash (float): Initial portfolio cash value.
            method (str): Optimization method to use for weight calculation ("pypfopt" or "scipy").
        
        Returns:
            pd.DataFrame: DataFrame indexed by date containing portfolio value, cash, asset weights, and share allocations for each rebalancing step.
        """
        # Validate date range
