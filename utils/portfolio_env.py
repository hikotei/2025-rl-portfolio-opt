...
class PortfolioEnv(gym.Env):
    ...
    def __init__(
        ...
        eta: float = 1 / 252,  # smoothing parameter for DSR
    ):
        """
        Initialize the portfolio environment with historical data and parameters.

        Think of this as setting up a trading simulator with real market data.
        For example, if you want to train an AI to trade stocks from 2020-2023,
        you'd provide the price and return data from that period.

        Args:
            returns_df: DataFrame of asset returns
            prices_df: DataFrame of asset prices
            vol_df: DataFrame of volatility metrics
            window_size: Number of days to look back for state
            transaction_cost: Cost per trade as a fraction
            initial_balance: Starting portfolio value
            reward_scaling: Scaling factor for rewards
            eta: Smoothing parameter for DSR
        """
        super().__init__()
        ...
        # [returns_window, vol_features, current_weights]
    ...
    def calc_metrics(self, portfolio_values: Union[List[float], np.ndarray, pd.Series], 
        ...
        sortino_ratio = np.sqrt(252) * np.mean(excess_returns) / np.std(negative_returns) if len(negative_returns) > 0 else 0
        ...
        omega_ratio = np.sum(positive_returns - threshold) / abs(np.sum(negative_returns - threshold)) if len(negative_returns) > 0 else float('inf')
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        # Convert action to portfolio weights using softmax to ensure they sum to 1
        exp_action = np.exp(action)
        self.weights = exp_action / np.sum(exp_action)
        self.previous_weights = self.weights.copy()

        # Calculate transaction costs
        # cost = self.transaction_cost * np.sum(np.abs(action - self.weights))
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        This is like gathering all the information a trader would look at before making a decision.
        For example, it includes:
        - Last 60 days of returns for each stock
